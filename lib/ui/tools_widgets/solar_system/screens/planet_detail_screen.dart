...
class PlanetDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final planetData =
        Provider.of<Planets>(context).getSinglePlanetData(planetName);
    return Scaffold(
...
Widget mainContent(double height, double width, BuildContext context,
    String planetName, Planet data) {
  ...
  if (data.timeToOrbitSun < 1) {
    timePreiod = Text(
      "Time to orbit Sun : $orbitDays days",
      style: style,
    );
  } else {
    timePreiod = Text(
      "Time to orbit Sun : ${data.timeToOrbitSun} year",
      style: style,
    );
  }
  ...
  if (data.dictanceFromEarth < 10) {
    distanceFromEarth = Text(
      "Distance from Earth : ${data.dictanceFromEarth} billion km",
      style: style,
    );
  } else {
    distanceFromEarth = Text(
      "Distance from Earth : ${data.dictanceFromEarth} million km",
      style: style,
    );
  }
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Column(
                ...
                children: <Widget>[
                  Text(
                    "Planet No. : ${data.positionInSolarSystem}",
                    style: style,
                  ),
                  ...
                  timePreiod,
                  Text(
                    "Moons : ${data.noOfMoons}",
                    style: style,
                  ),
                  Text(
                    "Avg. Temperature : ${data.avgTemp} Kelvin",
                    style: style,
                  )
                ],
              ),
